...
STAGE2_PATH = WORK_DIR / "stage2_next.txt"
...
# Stage-1 pair evidence carried into stage 2, so it can be recombined with group context.
PAIR_EVIDENCE = ["n_tset", "n_ratio", "c_jw", "k_ratio", "a_tset", "num_jac", "num_shared", "num_first_eq",
                 "addr_word_jac", "legal_eq", "state_eq", "pf_score", "pf_gap", "n_tset_gap", "a_tset_gap"]


def split_cands(split: str) -> pl.DataFrame:
    """Candidate pairs with context, in the same regime the stage-1 scores were produced in."""
    if split == "train":
        return load_train_cands(load_ground_truth("train"))
    return add_global_context(pl.read_parquet(work_path(split, "cands.parquet")))


def group_features(pred: pl.DataFrame, split: str, cands: pl.DataFrame | None = None) -> pl.DataFrame:
    ...
    # Stage-1 evidence for each in-play pair.
    if cands is None:
        cands = split_cands(split)
    s1_full = pl.read_parquet(work_path(split, "s1.parquet"))
    t_full = pl.read_parquet(work_path(split, "t.parquet"))
    ev = []
    keys = Q.select("tid", "s1")
    for a in range(0, len(keys), step):
        part = cands.join(keys.slice(a, step), on=["tid", "s1"], how="semi")
        ev.append(build(part, s1_full, t_full).select("tid", "s1", *PAIR_EVIDENCE))
    del s1_full, t_full
    Q = Q.join(pl.concat(ev), on=["tid", "s1"], how="left")
    print(f"[group] pair evidence done ({time.time() - t0:.0f}s)")

    # Orphan cohesion: how well the S1's confident members match the S1 itself. Orphaned groups
    # (records of an entity missing from source 1) agree with each other but only loosely with
    # the look-alike S1 they are drawn to.
    is_member = (pl.col("rank") == 1) & (pl.col("p") >= CONF_P)
    mem = Q.filter(is_member).group_by("s1").agg(
        pl.len().alias("_m"), pl.col("n_tset").sum().alias("_sn"), pl.col("a_tset").sum().alias("_sa"),
        pl.col("num_jac").sum().alias("_sj"), pl.col("n_tset").min().alias("grp_name_min"))
    Q = Q.join(mem, on="s1", how="left").with_columns(pl.col("_m").fill_null(0))
    own = is_member.cast(pl.Float32)
    denom = (pl.col("_m") - own).clip(lower_bound=1)
    Q = Q.with_columns(
        ((pl.col("_sn") - own * pl.col("n_tset")) / denom).alias("grp_name_to_s1"),
        ((pl.col("_sa") - own * pl.col("a_tset")) / denom).alias("grp_addr_to_s1"),
        ((pl.col("_sj") - own * pl.col("num_jac")) / denom).alias("grp_num_to_s1"),
    ).with_columns(
        # Null when the S1 has no other confident members.
        [pl.when(pl.col("_m") - own > 0).then(pl.col(c)).otherwise(None).alias(c)
         for c in ("grp_name_to_s1", "grp_addr_to_s1", "grp_num_to_s1")]
    ).with_columns(
        (pl.col("sib_name_mean") - pl.col("grp_name_to_s1")).alias("orphan_name"),
        (pl.col("sib_addr_mean") - pl.col("grp_addr_to_s1")).alias("orphan_addr"),
        (pl.col("n_tset") - pl.col("grp_name_to_s1")).alias("self_vs_grp_name"),
    ).drop("_m", "_sn", "_sa", "_sj")
...
FEATURES2 = [
    ...
    "grp_name_to_s1", "grp_addr_to_s1", "grp_num_to_s1", "grp_name_min", "orphan_name", "orphan_addr",
    "self_vs_grp_name",
] + PAIR_EVIDENCE
...
def train() -> None:
    ...
    feats = group_features(pred, "train", cands).join(links, on=["tid", "s1"], how="left").with_columns(
        pl.col("y").fill_null(0))
    del cands
    ...
    val2.write_parquet(work_path("train", "val_pred2_next.parquet"))
    print("[stage2] stage 2:")
    tune(val2, *ctx, out="decision2_next.json")
...
def test() -> None:
    ...
    rescore(pred, group_features(pred, "test"), model).write_parquet(work_path("test", "pred2_next.parquet"))
